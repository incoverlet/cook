...
import kotlinx.android.synthetic.main.activity_curry_menu3.*
import kotlinx.android.synthetic.main.activity_garlicbread_menu3.*
import kotlinx.android.synthetic.main.activity_mixedrice_menu3.*
import java.util.*
...
class mixedrice_menu3 : AppCompatActivity() {
    @RequiresApi(Build.VERSION_CODES.N)
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_mixedrice_menu3)
        //인텐트 넘기기

        mixedrice3_textView3.text = "[1 단계]. 재료 손질"	// TextView 세팅
        mixedrice3_textView4.text = "햄, 양파, 당근 모든 재료를 \n 한입 크기로 썰어 줍니다."	// TextView 세팅

        mixedrice3_button2.setOnClickListener {
            if(flag == 1){
        ...
        timerTask = kotlin.concurrent.timer(period = 1000) {	// timer() 호출
            ...
            // UI조작을 위한 메서드
            runOnUiThread {
                mixedrice3_textView5.text = "$a : $b"	// TextView 세팅
                mixedrice3_textView6.text = "$c : $d"	// TextView 세팅
            }
